...
std::int64_t CalcPart1(const Input &input) {
  ...
  for (const auto &sequence : input) {
    ...
    for (auto pos = 0; pos < patterns.size() - 1; pos++) {
      auto &to_modify = *(patterns.rbegin() + pos + 1);
      auto &to_reference = *(patterns.rbegin() + pos);
...
std::int64_t CalcPart2(const Input &input) {
  ...
  for (const auto &sequence : input) {
    ...
    for (auto pos = 0; pos < patterns.size() - 1; pos++) {
      ...
      const auto new_value = (*to_modify.begin() - *to_reference.begin());
      to_modify.insert(to_modify.begin(), new_value);
    }

    results.push_back(*patterns[0].begin());
  }
